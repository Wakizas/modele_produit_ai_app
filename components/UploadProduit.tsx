...
const extractFramesFromVideo = (videoFile: File, frameCount: number): Promise<{ base64: string, file: File }[]> => {
    return new Promise((resolve, reject) => {
        ...
        video.onloadedmetadata = () => {
            ...
            if (duration === 0) {
              reject(new Error("Video duration is 0, cannot extract frames."));
              return;
            }
            ...
            video.onseeked = () => {
                if (capturedFrames < frameCount && context) {
            ...
            for (let i = 0; i < frameCount; i++) {
                // Ensure we don't seek to the exact same time for very short videos
                const time = Math.min(duration * (i / (frameCount - 1)) - (i > 0 ? 0.01 : 0), duration);
                captureFrame(i === 0 ? 0.1 : time); // Start slightly after 0 to ensure first frame is loaded
            }
        };
